...
CONFIG_SYS_SPL_ARGS_ADDR="(PHYS_DRAM_1 + 0x100)"
CONFIG_SPL_OS_BOOT=y
CONFIG_CMD_MISC=y
CONFIG_SPL_LIBCOMMON_SUPPORT=y
CONFIG_PHY_GIGE=y
CONFIG_ENV_OFFSET="(892 << 10)"
...
CONFIG_CMD_NAND=y
CONFIG_CMD_SPL_WRITE_SIZE=0x1000
...
CONFIG_CMD_EXT2=y
CONFIG_CMD_EXT4=y
CONFIG_BOOTCOMMAND="run findfdt; mmc dev ${mmcdev}; if mmc rescan; then echo SD/MMC found on device ${mmcdev};if run loadbootenv; then echo Loaded environment from ${bootenv};run importbootenv;fi;if test -n $uenvcmd; then echo Running uenvcmd ...;run uenvcmd;fi;if run loaduimage; then run loadfdt;run mmcboot;fi;else run spiboot;fi;"
...
CONFIG_SYS_SPL_MALLOC_START=0x80a08000
...
CONFIG_EXTRA_ENV_SETTINGS="loadaddr=0x80200000\0fdtaddr=0x80F80000\0fdt_high=0xffffffff\0rdaddr=0x81000000\0bootdir=/boot\0bootfile=uImage\0fdtfile=undefined\0console=ttyO0,115200n8\0optargs=\0mtdids=" MTDIDS_DEFAULT "\0mtdparts=" MTDPARTS_DEFAULT "\0dfu_alt_info_mmc=" DFU_ALT_INFO_MMC "\0dfu_alt_info_emmc=rawemmc mmc 0 3751936\0dfu_alt_info_nand=" DFU_ALT_INFO_NAND "\0mmcdev=0\0mmcroot=/dev/mmcblk0p2 ro\0mmcrootfstype=ext4 rootwait\0bootpart=0:2\0nandroot=ubi0:rootfs rw ubi.mtd=7,2048\0nandrootfstype=ubifs rootwait=1\0nandsrcaddr=0x280000\0nandimgsize=0x500000\0rootpath=/export/rootfs\0nfsopts=nolock\0static_ip=${ipaddr}:${serverip}:${gatewayip}:${netmask}:${hostname}::off\0ramroot=/dev/ram0 rw ramdisk_size=65536 initrd=${rdaddr},64M\0ramrootfstype=ext2\0mmcargs=setenv bootargs console=${console} ${optargs} root=${mmcroot} rootfstype=${mmcrootfstype}\0nandargs=setenv bootargs console=${console} ${optargs} root=${nandroot} rootfstype=${nandrootfstype}\0spiroot=/dev/mtdblock4 rw\0spirootfstype=jffs2\0spisrcaddr=0xe0000\0spiimgsize=0xFA0000\0spibusno=0\0spiargs=setenv bootargs console=${console} ${optargs} root=${spiroot} rootfstype=${spirootfstype}\0netargs=setenv bootargs console=${console} ${optargs} root=/dev/nfs nfsroot=${serverip}:${rootpath},${nfsopts} rw ip=dhcp\0bootenv=uEnv.txt\0loadbootenv=load mmc ${mmcdev} ${loadaddr} ${bootenv}\0importbootenv=echo Importing environment from mmc ...; env import -t $loadaddr $filesize\0ramargs=setenv bootargs console=${console} ${optargs} root=${ramroot} rootfstype=${ramrootfstype}\0loadramdisk=load mmc ${mmcdev} ${rdaddr} ramdisk.gz\0loaduimage=load mmc ${bootpart} ${loadaddr} ${bootdir}/${bootfile}\0loadfdt=load mmc ${bootpart} ${fdtaddr} ${bootdir}/${fdtfile}\0mmcboot=echo Booting from mmc ...; run mmcargs; bootm ${loadaddr} - ${fdtaddr}\0nandboot=echo Booting from nand ...; run nandargs; nand read ${loadaddr} ${nandsrcaddr} ${nandimgsize}; bootm ${loadaddr}\0spiboot=echo Booting from spi ...; run spiargs; sf probe ${spibusno}:0; sf read ${loadaddr} ${spisrcaddr} ${spiimgsize}; go ${loadaddr}\0netboot=echo Booting from network ...; setenv autoload no; dhcp; tftp ${loadaddr} ${bootfile}; tftp ${fdtaddr} ${fdtfile}; run netargs; bootm ${loadaddr} - ${fdtaddr}\0ramboot=echo Booting from ramdisk ...; run ramargs; bootm ${loadaddr} ${rdaddr} ${fdtaddr}\0findfdt=if test $board_name = A335BONE; then setenv fdtfile am335x-bone.dtb; fi; if test $board_name = A335BNLT; then setenv fdtfile am335x-boneblack.dtb; fi; if test $board_name = A33515BB; then setenv fdtfile am335x-evm.dtb; fi; if test $board_name = A335X_SK; then setenv fdtfile am335x-evmsk.dtb; fi; if test $fdtfile = undefined; then echo WARNING: Could not determine device tree to use; fi; \0"
...
CONFIG_OMAP=y
CONFIG_FS_EXT4=y
CONFIG_SPL_MAX_SIZE="(0x4030C000 - CONFIG_SPL_TEXT_BASE)"
